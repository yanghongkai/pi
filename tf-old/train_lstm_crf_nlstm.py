...
tf.app.flags.DEFINE_integer('num_tags',16,'dependency parser label')
...
class Model:
    ...
    def inference(self,X,reuse=None,trainMode=True):
        ...
        lstm_cell_fw=tf.nn.rnn_cell.LSTMCell(self.numHidden)
        if trainMode:
            lstm_cell_fw=tf.nn.rnn_cell.DropoutWrapper(lstm_cell_fw,output_keep_prob=0.5)
        cell_fw=tf.nn.rnn_cell.MultiRNNCell([lstm_cell_fw]*FLAGS.num_layers,state_is_tuple=True)
        with tf.variable_scope("rnn_fwbw",reuse=reuse) as scope:
            ...
            output=tf.reshape(output,[-1,self.numHidden])
            # output [100*80,200]
            if trainMode:
